...
pipeline {
    ...
    stages {
        ...
        steps {
           ...
           sshPublisher(publishers: [sshPublisherDesc(configName: 'host', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: '', execTimeout: 120000, flatten: false, makeEmptyDirs: true, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: 'MM-DD-YYYY/build-$BUILD_NUMBER-$GIT_COMMIT', remoteDirectorySDF: true, removePrefix: '', sourceFiles: '*.zip, dist/1.x/PLATOTerm*, dist/2.x/PLATOTerm*', useSftpForExec: true)], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
           }
        }
    }
}
